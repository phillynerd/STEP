...
#### Summary Statistics
All data from `r format(as.Date(params$StartDate, format = "%m/%d/%Y"), "%B %Y")` through `r format(as.Date(params$EndDate, format = "%m/%d/%Y"), "%B %Y")` for all schools in program

* Total Number of Schools: `r length(unique(STEPfinal$SchoolName[is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "School" & STEPfinal$EncounterDate < as.Date(params$EndDate, "%m/%d/%Y")]))` 
* Total Number of School-Specific Activities: `r nrow(STEPfinal[is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "School" & STEPfinal$EncounterDate < as.Date(params$EndDate, "%m/%d/%Y"),])` 
* Total Number of Child-Specific Activities: `r nrow(STEPfinal[is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "Child" & STEPfinal$EncounterDate < as.Date(params$EndDate, "%m/%d/%Y"),])` 
* Number of Unique Children Served: `r length(unique(STEPfinal$ChildID[is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "Child"]))`
...
STEPfinal %>% 
  ...
  group_by(YMencounter, TaskCat, TaskType_Cat) %>% 
...
STEPfinal %>% 
  ...
  kable(booktabs = T, escape = F, linesep = "",
        caption = "Monthly Summary Data for All Programs") %>% 
...
STEPfinal %>% 
  mutate(YMencounter = as.yearmon(EncounterDate)) %>% 
  group_by(YMencounter, TaskCat, TaskType) %>% 
  summarize(TotalTime = round(sum(TimeElapsed)/3600,2), #Note: 4 cases where timeelapsed is NA
            TotalEntries = n(),
            NDistinctKids = n_distinct(ChildID)) %>% 
  mutate(NDistinctKids = ifelse(TaskCat == "Child", NDistinctKids, NA)) %>% 
  ...
  kable(longtable = T, booktabs = T, escape = F, linesep = "", 
        caption = "Detailed Data for All Programs") %>% 
