---
params:
    Staff: "Abernathy, Jennifer" 
    StartDate: "08/01/2019"
    EndDate: "10/01/2019"
title: "STEP Metrics: August 2019 - October 2019"
...
```
# Staff Member: `r params$Staff`
## School: `r STEPfinal %>% filter(ClinFullName == params$Staff) %>% distinct(SchoolName) %>% pull(SchoolName)`
...
* Total Number of School-Specific Activities: `r nrow(STEPfinal[STEPfinal$ClinFullName == params$Staff & is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "School" & STEPfinal$EncounterDate <= as.Date(params$EndDate, format = "%m/%d/%Y"),])` 
* Total Number of Child-Specific Activities: `r nrow(STEPfinal[STEPfinal$ClinFullName == params$Staff & is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "Child" & STEPfinal$EncounterDate <= as.Date(params$EndDate, format = "%m/%d/%Y"),])` 
* Number of Unique Children Served: `r length(unique(STEPfinal$ChildID[STEPfinal$ClinFullName == params$Staff & is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "Child" & STEPfinal$EncounterDate <= as.Date(params$EndDate, format = "%m/%d/%Y")]))`
...
STEPfinal %>% 
  ...
  group_by(ClinFullName, YMencounter, TaskCat) %>% 
  summarize(TotalTime = round(sum(TimeElapsed)/3600,2), #Note: 4 cases where timeelapsed is NA
            TotalEntries = n(),
            NDistinctKids = n_distinct(ChildID)) %>% 
  mutate(NDistinctKids = ifelse(TaskCat == "Child", NDistinctKids, NA)) %>% 
  filter(ClinFullName == params$Staff) %>% #, YMencounter == MonthParam) %>% 
  arrange(YMencounter) %>% 
  ungroup() %>% 
...
STEPfinal %>% 
  mutate(YMencounter = as.yearmon(EncounterDate)) %>% 
  group_by(ClinFullName, YMencounter, TaskCat, TaskType) %>% 
  summarize(TotalTime = round(sum(TimeElapsed)/3600,2), #Note: 4 cases where timeelapsed is NA
            TotalEntries = n(),
            NDistinctKids = n_distinct(ChildID)) %>% 
  mutate(NDistinctKids = ifelse(TaskCat == "Child", NDistinctKids, NA)) %>% 
