---
params:
  ClinRole: "Clinical Social Work Coordinator"
  StartDate: "08/01/2019"
  EndDate: "10/01/2019"
title: "STEP Metrics Across All Schools: August 2019 - September 2019"
...
```
# STEP Staff Role: `r params$ClinRole`s
...
* Total Number of School-Specific Activities: `r nrow(STEPfinal[STEPfinal$ClinRole2 == params$ClinRole & is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "School" & STEPfinal$EncounterDate < as.Date(params$EndDate, "%m/%d/%Y"),])` 
* Total Number of Child-Specific Activities: `r nrow(STEPfinal[STEPfinal$ClinRole2 == params$ClinRole & is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "Child" & STEPfinal$EncounterDate < as.Date(params$EndDate, "%m/%d/%Y"),])` 
* Number of Unique Children Served: `r length(unique(STEPfinal$ChildID[STEPfinal$ClinRole2 == params$ClinRole & is.na(STEPfinal$TaskType) == F & STEPfinal$TaskCat == "Child" & STEPfinal$EncounterDate < as.Date(params$EndDate, "%m/%d/%Y")]))`
...
STEPfinal %>% 
  ...
  labs(title = "Time Spent on School-Specific Tasks Across All Schools",
       subtitle = paste0("Staff Role: ", params$ClinRole),
       x = "Month of Service",
       y = "Total Time (Hours)",
       fill = "Task Category",
...
STEPfinal %>% 
  ...
  geom_label(aes(label = paste0(TotalTime,"H; N = ", NDistinctKids)), 
            position = position_stack(vjust = .5), 
            size = 2.5, color = "black", alpha = .3, fill = "white", 
            show.legend = NA, label.size = 0, label.padding = unit(.1, "lines")) +
  scale_x_yearmon(breaks = seq(from = as.yearmon(as.Date(params$StartDate, format = "%m/%d/%Y")), 
                               to = as.yearmon(as.Date(params$EndDate, format = "%m/%d/%Y")),
                               by = 1/12)) +
  ...
  #theme(axis.text.x = element_text(angle= 45, hjust = 1)) +
  labs(title = "Time Spent on Child-Specific Tasks Across All Schools",
       subtitle = paste0("Staff Role: ", params$ClinRole),
       x = "Month of Service",
       y = "Total Time (Hours)",
       fill = "Task Category",
       caption = "'Therapy' and 'Collateral Contacts' were aggregated from more detailed datan\
...
STEPfinal %>% 
  ...
  group_by(ClinRole2, YMencounter, TaskCat) %>% 
  ...
  filter(ClinRole2 == params$ClinRole) %>% #, YMencounter == MonthParam) %>% 
  arrange(YMencounter) %>% 
  ungroup() %>% 
  ...
  kable(booktabs = T, escape = F, linesep = "",
        caption = paste0("Monthly Summary Data for ", params$ClinRole, " Across All Schools")) %>% 
...
STEPfinal %>% 
  mutate(YMencounter = as.yearmon(EncounterDate)) %>% 
  group_by(ClinRole2, YMencounter, TaskCat, TaskType) %>% 
  summarize(TotalTime = round(sum(TimeElapsed)/3600,2), #Note: 4 cases where timeelapsed is NA
            TotalEntries = n(),
            NDistinctKids = n_distinct(ChildID)) %>% 
  mutate(NDistinctKids = ifelse(TaskCat == "Child", NDistinctKids, NA)) %>% 
  ...
  kable(longtable = T, booktabs = T, escape = F, linesep = "", 
        caption = paste0("Detailed Data for ", params$ClinRole, " Across All Schools")) %>% 
